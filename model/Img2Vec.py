...
class Img2Vec(pl.LightningModule):
    ...
    def training_step(self, batch, batch_no):
        ...
        self.log("train_acc_step", self.train_acc, on_step = True, on_epoch = False, prog_bar = True, logger = True)
        self.log(
            "train_acc_epoch", 
            self.train_acc, 
            on_step = False, 
...
#     """
#     def __init__(self, character_classes: list, diacritic_classes: list, half_character_classes:list,
#                 backbone: pl.LightningModule or nn.Module, optimizer = torch.optim.Adam, lr= 1e-3, threshold = 0.5, 
#                 rep_dim = 2048, weight_decay = 0.01, activation = nn.ReLU()):
#         super().__init__()
#         self.character_classes = character_classes # 54 + 10 + 9 + 27 also counting numbers vowels and chinh
#         self.diacritic_classes = diacritic_classes
...
#         self.backbone = backbone
#         self.activation = activation
#         self.char_rep_layer = nn.Sequential(
#                             self.activation,
#                             nn.Linear(
#                                 in_features = self.backbone.model.fc.out_features,
#                                 out_features = self.rep_dim,
...
#                             self.activation
#                         )
#         self.diac_rep_layer = nn.Sequential(
#                             self.activation,
#                             nn.Linear(
#                                 in_features= self.backbone.model.fc.out_features,
#                                 out_features = self.rep_dim,
...
#     def forward(self, x):
#         x = self.backbone(x)
#         char_rep = self.char_rep_layer(x)
#         diac_rep = self.diac_rep_layer(x)
#         half_char_logits = self.half_character_head(char_rep)
#         char_logits = self.character_head(char_rep)
#         diac_logits = self.diacritic_head(diac_rep)
...
#         self.half_character_classes = half_character_classes # aadhe akshar
#         self.rep_dim = rep_dim
#         self.char_backbone = backbone()
#         self.diac_backbone = backbone()
#         self.activation = activation
#         self.char_rep_layer = nn.Sequential(
#                             self.activation,
#                             nn.Linear(
#                                 in_features = self.char_backbone.model.fc.out_features,
#                                 out_features = self.rep_dim,
#                                 bias = True
#                             ),
#                             self.activation
...
#                             self.activation,
#                             nn.Linear(
#                                 in_features= self.diac_backbone.model.fc.out_features,
#                                 out_features = self.rep_dim,
#                                 bias = True
#                             ),
#                             self.activation
...
#         char_feat = self.char_backbone(i)
#         diac_feat = self.diac_backbone(i)
#         # get the hidden representation
#         char_rep = self.char_rep_layer(char_feat)
#         diac_rep = self.diac_rep_layer(diac_feat)
#         # pass through the respective classification heads
#         half_char_logits = self.half_character_head(char_rep)
#         char_logits = self.character_head(char_rep)
#         diac_logits = self.diacritic_head(diac_rep)
