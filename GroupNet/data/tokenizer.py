...
class HindiTokenizer:
    ...
    def __init__(self, svar:list, vyanjan:list, matras:list, ank:list, chinh:list,
                halanth:str, threshold:float= 0.5, max_grps:int= 25):
        """
        Args:
    ...
    def grp_sanity(self, label:str, grps:tuple)-> bool:
        ...
        for grp in grps:
            ...
            i = 0
            while i < len(grp):
                if i + 1 < len(grp) and self.halanth == grp[i + 1] and grp[i] in self.h_c_set and h_c_count > 0:
                    h_c_count -= 1
                    i += 1
    ...
    def grp_class_encoder(self, grp: str)-> Tuple[int, int, int, Tensor]:
        ...
        for i, char in enumerate(grp):
            halfer_cntr = 0
            if char in self.h_c_set and i + 1 < len(grp) and grp[i+1] == self.halanth:
...
class DevanagariTokenizer:
    ...
    def __init__(self, svar:list, vyanjan:list, matras:list, ank:list, chinh:list,
        ...
        self.ank = ank
        self.chinh = chinh
        self.nukthas = ['क़', 'ख़', 'ग़', 'ज़', 'ड़', 'ढ़', 'ऱ'] # half-char cannot contain matras so added here
        self.halanth = halanth
