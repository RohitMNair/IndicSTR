...
class ViTSTR(pl.LightningModule):
    """
    ...
    def __init__(self, emb_path:str, half_character_classes:list, full_character_classes:list,
        ...
        self.weight_decay = weight_decay
        self.pct_start = warmup_pct
        self.max_grps = (self.image_size // self.patch_size)**2 + 1
        ...
        # self.d_pos_weight = torch.full([self.num_d_classes,], 1.)
        # self.d_pos_weight[self.tokenizer.pad_id] = 0
    ...
    def _log_tb_images(self, viz_batch:Tensor, pred_labels:tuple, gt_labels:Optional[tuple], mode:str= "test") -> None:
        """
        Function to display a batch and its predictions to tensorboard
        Args:
        - viz_batch (Tensor): images of a batch to be visualized
        ...
        if mode == "train" or mode == "val":
            ...
            # Log the images (Give them different names)
            for img_idx in range(viz_batch.shape[0]):
                tb_logger.add_image(f"{mode}/pred-{pred_labels[img_idx]}_gt-{gt_labels[img_idx]}", viz_batch[img_idx], self.global_step)
        
        else:
            assert pred_labels is not None, "pred_labels must be provided in training mode"
            for img_idx in range(viz_batch.shape[0]):
                tb_logger.add_image(f"{mode}/pred-{pred_labels[img_idx]}_{img_idx}", viz_batch[img_idx], 0)
    
    def configure_optimizers(self)-> dict:
        optmizer = Adam(params= self.parameters(), lr= self.lr, weight_decay= self.weight_decay)
        lr_scheduler = OneCycleLR(
            optimizer= optmizer,
            max_lr= self.lr,
            total_steps= int(self.trainer.estimated_stepping_batches), # gets the max training steps
    ...
    def training_step(self, batch, batch_no)-> Tensor:
        ...
        if batch_no % 100000:
            ...
            self._log_tb_images(imgs, pred_labels= pred_labels, gt_labels= labels, mode="train")
        # On step logs for proggress bar display
        log_dict_step = {
            "train_loss_step": loss,
            "train_wrr_step": self.train_wrr,
    ...
    def validation_step(self, batch, batch_idx)-> None:
        ...
        if batch_idx % 100000:
            pred_labels = self.tokenizer.decode((h_c_2_logits, h_c_1_logits, f_c_logits, d_logits))
            self._log_tb_images(imgs, pred_labels=labels, gt_labels= pred_labels, mode= "val")
        # On epoch only logs
        log_dict_epoch = {
            "val_loss_epoch": loss,
            "val_half_character2_acc": self.val_h_c_2_acc,
    ...
    def test_step(self, batch, batch_idx)-> None:
        ...
        self.test_wrr((h_c_2_logits, h_c_1_logits, f_c_logits, d_logits),\
                      (h_c_2_targets, h_c_1_targets, f_c_targets, d_targets))
        labels = self.tokenizer.decode((h_c_2_logits, h_c_1_logits, f_c_logits, d_logits))
        self._log_tb_images(imgs, pred_labels= labels, gt_labels=None, mode= "test")
        # On epoch only logs
        log_dict_epoch = {
            "test_loss_epoch": loss,
            "test_half_character2_acc": self.test_h_c_2_acc,
...
class FocalSTR(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_no)-> None:
        ...
        if batch_no % 100000:
            pred_labels = self.tokenizer.decode((h_c_2_logits, h_c_1_logits, f_c_logits, d_logits))            
            self._log_tb_images(imgs, pred_labels= pred_labels, gt_labels= labels, mode= "val")
        # On epoch only logs
        log_dict_epoch = {
            "val_loss_epoch": loss,
            "val_half_character2_acc": self.val_h_c_2_acc,
    ...
    def test_step(self, batch, batch_idx)-> None:
        ...
        if batch_idx % 100000:
            pred_labels = self.tokenizer.decode((h_c_2_logits, h_c_1_logits, f_c_logits, d_logits))            
            self._log_tb_images(imgs, pred_labels= pred_labels, gt_labels= None, mode= "test")
